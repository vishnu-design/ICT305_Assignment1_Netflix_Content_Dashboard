...
def create_treemap_data(df):
    ...
    # Create hierarchical data: Country -> Type -> Count
    treemap_data = []
    
    for country in df['primary_country'].value_counts().head(10).index:
        country_data = df[df['primary_country'] == country]
        total_country = len(country_data)
        
        for content_type in ['Movie', 'TV Show']:
            type_data = country_data[country_data['type'] == content_type]
            count = len(type_data)
            
            if count > 0:  # Only add if there's content
                treemap_data.append({
                    'ids': f"{country}_{content_type}",
                    'labels': f"{content_type}\n({count})",
                    'parents': country,
                    'values': count
                })
...
def main():
    ...
    # Row 1: Content distribution and Tree Map
    ...
    with col2:
        st.subheader("Content by Country & Type (Tree Map)")
        try:
            treemap_df = create_treemap_data(filtered_df)
            ...
            if len(treemap_df) > 0:
                ...
                st.info("Not enough data to create tree map with current filters.")
        except Exception as e:
            st.error(f"Error creating tree map: {str(e)}")
            # Fallback to a simple genre treemap
            genre_counter = create_genre_data(filtered_df)
            top_genres = dict(genre_counter.most_common(15))
            
            fig_treemap_fallback = px.treemap(
                names=list(top_genres.keys()),
                values=list(top_genres.values()),
                title="Top Genres (Tree Map)"
            )
            st.plotly_chart(fig_treemap_fallback, use_container_width=True)
    
    # Row 2: Content trends and geographic analysis
    col1, col2 = st.columns(2)
    ...
    # Row 3: Rating and Genre Tree Map
    col1, col2 = st.columns(2)
    ...
    with col2:
        ...
        try:
            ...
            if len(top_genres) > 0:
                ...
                fig_genre_treemap.update_traces(
                    textinfo="label+value+percent root",
                    textfont_size=12,
                    hovertemplate='<b>%{label}</b><br>Count: %{value}<br>%{percentRoot} of total<extra></extra>'
                )
                st.plotly_chart(fig_genre_treemap, use_container_width=True)
        except Exception as e:
            st.error(f"Error creating genre tree map: {str(e)}")
    
    # Row 4: Genre analysis (bar charts)
